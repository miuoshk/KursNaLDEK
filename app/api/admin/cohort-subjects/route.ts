...
import { loadAdminCohortSubjectPopularity } from "@/features/admin/server/loadAdminCohortSubjectPopularity";
...
export async function GET(request: Request) {
  ...
  try {
    const series = await loadAdminCohortSubjectPopularity({ track, year, range });
    return NextResponse.json(series, {
      headers: { "Cache-Control": "private, max-age=30" },
    });
  } catch (e) {
    console.error("[api/admin/cohort-subjects]", e);
