...
export async function loadQuestionsByIdsOrdered(
  ...
  for (let offset = 0; offset < ids.length; offset += IN_CHUNK) {
    ...
    const query = options?.includeSourceMeta
      ? base
          .select(
            "id, topic_id, text, options, correct_option_id, explanation, explanation_blocks, source_code, image_url, disable_option_shuffle, source, repeat_count, first_seen_session, source_exam, question_concepts(concept_id, relation, weight, concepts(name)), topics ( name, knowledge_card )",
          )
          .in("id", slice)
          .eq("is_active", true)
      : base
          .select(
            "id, topic_id, text, options, correct_option_id, explanation, explanation_blocks, source_code, image_url, disable_option_shuffle, question_concepts(concept_id, relation, weight, concepts(name)), topics ( name, knowledge_card )",
          )
          .in("id", slice)
          .eq("is_active", true);
