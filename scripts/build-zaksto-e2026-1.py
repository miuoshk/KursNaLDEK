#!/usr/bin/env python3
"""Konwersja ZAKSTO TXT → TXT fabryki + SQL chorób zakaźnych STOMA.

Nie pisze SQL-a ręcznie: ten skrypt jest jedynym źródłem INSERT-ów.
ID w bazie: zaksto-z2026-1-{NR_LOKALNY} (jak z2025-1).
topic_id: ZAKAZ-NN (produkcja; CHZ-* usunięte).
"""
...
TEMAT_MAP = {n: f"ZAKAZ-{n:02d}" for n in range(1, 24)}

BATCH_LABEL = "z2026-1"
SOURCE_EXAM = "Egzamin SUM 2026 (ZAKSTO e2026-1)"
ZAK_TOPICS = tuple(TEMAT_MAP.values())

EXPL_FIXES = {
    "ZAKSTO-846": (
        " Szczepionki przeciw HCV nie ma, immunoglobulina jest nieskuteczna. W oryginale pytania podano omyłkowo anty-HBV; przeciwciała te nie chronią przed reinfekcją.",
        " Szczepionki przeciw HCV nie ma, immunoglobulina jest nieskuteczna. Przeciwciała anty-HCV nie chronią przed reinfekcją.",
    ),
    "ZAKSTO-857": (
        " Sam wiek nie jest wskazaniem. Autor klucza wskazał dzieci przedszkolne i szkolne, ale ta grupa w PSO figuruje.",
        " Sam wiek nie jest wskazaniem.",
    ),
    "ZAKSTO-883": (
        " Autor klucza wskazał podróżnego, u którego szczepienie jest jednak zalecane.",
        "",
    ),
}


def db_id(nr: str) -> str:
    return f"zaksto-z2026-1-{int(nr)}"
...
def parse_source(raw: str) -> list[dict]:
    ...
    for index, block in enumerate(blocks, 1):
        ...
        item["db_id"] = db_id(item["nr"] or str(index))
        items.append(item)
    return items
...
def build_sql(items: list[dict]) -> str:
    ...
    topic_list = ", ".join(f"'{t}'" for t in sorted({i["topic_id"] for i in items}))
    header = f"""-- ============================================================
-- BATCH: {BATCH_LABEL}  ·  subject_id=stoma-zakazne  ·  STOMA (tracks=NULL)
...
"""
    out_rows = [
        row[:-1] + f",\n   '{BATCH_LABEL}', '{SOURCE_EXAM}', '2026')"
        for row in rows
    ]
    return header + ",\n".join(out_rows) + footer


def main() -> int:
    src = Path("/Users/miuoshk/Downloads/ZAKSTO-e2026-1.txt")
    db_path = Path("/tmp/zakaz-db-stems.json")
    factory_path = ROOT / "exports" / "zaksto-e2026-1-factory.txt"
    sql_path = ROOT / "exports" / "zakazne-batch-stoma-2026-1.sql"
    json_path = ROOT / "exports" / "zakazne-batch-stoma-2026-1.json"
    audit_path = ROOT / "exports" / "zakazne-batch-stoma-2026-1-audit.md"
    ...
    if db_path.exists():
        ...
        for row in report["near"]:
            print(f"  NEAR[{row['sim']}] {row['srccode']} ↔ {row['match']['id']} ({row['match'].get('batch_label')})")
            print(f"         batch: {row['text'][:80]}")
            print(f"         db:    {row['match']['text'][:80]}")
    else:
        print("DEDUP: brak /tmp/zakaz-db-stems.json — pomijam")

    leftover = []
    for item in items:
        if (
            "FLAGA:" in item["explanation"]
            or "Autor klucza" in item["explanation"]
            or "W oryginale" in item["explanation"]
        ):
    ...
    lines = [
        ...
        "",
        f"- pytań w pliku: **{len(items)}** (ZAKSTO-843 … ZAKSTO-883)",
    ...
    if not report["near"] and not report["exact"]:
        lines.append("Brak exact/near względem aktywnych chorób zakaźnych.")
