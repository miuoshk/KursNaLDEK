/**
 * Jednorazowy insert batcha e_anat_2026/1.
 * ID: max seq z bazy + 1 per topic (jak CTE w SQL).
 */
...
const items = JSON.parse(
  readFileSync(resolve("exports/anatomia-batch-lek-2026-1.json"), "utf8"),
);
const supabase = createClient(url, key, { auth: { persistSession: false } });

const topicIds = [...new Set(items.map((q) => q.topic_id))];
async function fetchAllIds(supabase, topicIds) {
  const all = [];
  let from = 0;
  const page = 1000;
  while (true) {
...
const { count: dupBatch } = await supabase
  ...
  .eq("batch_label", "e_anat_2026/1");
if (dupBatch) {
  console.error("Batch e_anat_2026/1 już istnieje:", dupBatch);
  process.exit(1);
}
